...
export function HighDensityFlatA4Template({ invoice, profile, showGroups , publicUrl }: TemplateProps) {
      ...
      <div className="max-w-[210mm] w-full min-h-[297mm] mx-auto bg-white shadow-md print:shadow-none print:rounded-none print:border-none print:w-[210mm] print:max-w-[210mm] print:mx-0 print:min-h-[297mm] print:my-0">
        ...
        <div className="p-6 md:p-8 print:p-6">
          ...
          {(profile?.company_address || profile?.email || profile?.phone) && (
            <div className="mb-4 text-xs text-[#64748b] flex flex-wrap gap-x-6 gap-y-1 pb-3 border-b border-[#f1f5f9]">
          ...
          {/* Dense info strip */}
          <div className="grid grid-cols-1 sm:grid-cols-4 print:grid-cols-4 gap-3 sm:gap-4 mb-4 bg-[#f8fafc] p-3 rounded-lg border border-[#e2e8f0] text-sm">
            <div>
          ...
          <table className="w-full mb-4 text-sm">
            <thead>
              <tr className="bg-[#f8fafc]">
                <th className="text-left py-1.5 px-3 text-[10px] text-[#94a3b8] uppercase font-semibold border-b border-[#e2e8f0]">#</th>
                <th className="text-left py-1.5 px-3 text-[10px] text-[#94a3b8] uppercase font-semibold border-b border-[#e2e8f0]">Description</th>
                <th className="text-right py-1.5 px-3 text-[10px] text-[#94a3b8] uppercase font-semibold border-b border-[#e2e8f0] w-16">Qty</th>
                <th className="text-right py-1.5 px-3 text-[10px] text-[#94a3b8] uppercase font-semibold border-b border-[#e2e8f0] w-24">Rate</th>
                <th className="text-right py-1.5 px-3 text-[10px] text-[#94a3b8] uppercase font-semibold border-b border-[#e2e8f0] w-24">Amount</th>
              </tr>
            </thead>
                  ...
                  <React.Fragment key={gIdx}>
                    {group.name && (
                      <tr className="bg-slate-100 font-bold">
                        <td colSpan={5} className="py-1.5 px-3 text-xs text-[#0f172a] border-b border-[#e2e8f0] uppercase text-left font-semibold" style={{ fontFamily: 'Geist, sans-serif' }}>{group.name}</td>
                      </tr>
                    )}
                    {group.items.map((item, iIdx) => (
                      <tr key={iIdx} className={`border-b border-[#f1f5f9] ${iIdx % 2 === 0 ? '' : 'bg-[#fafbfc]'}`}>
                        <td className="py-1.5 px-3 text-[#94a3b8]">{iIdx + 1}</td>
                        <td className="py-1.5 px-3 text-[#0f172a]" style={{ fontFamily: 'Geist, sans-serif' }}>{item.name}</td>
                        <td className="py-1.5 px-3 text-right text-[#64748b]">{item.quantity}</td>
                        <td className="py-1.5 px-3 text-right text-[#64748b]">{formatMoney(item.unitPrice, sym)}</td>
                        <td className="py-1.5 px-3 text-right font-semibold text-[#0f172a]">{formatMoney(item.quantity * item.unitPrice, sym)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                ))
          ...
          <div className="mt-6 pt-4 border-t border-[#e2e8f0] flex flex-col md:flex-row print:flex-row justify-between items-start gap-4 text-sm w-full">
            {((profile?.bank_enabled ?? true) && (invoice.bank_name || profile?.bank_name)) ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 print:grid-cols-4 gap-4 flex-1">
