...
export function ModernPurpleTemplate({ invoice, profile, isPreview, showGroups, showGroupTotals, publicUrl }: TemplateProps) {
          ...
          <header className="flex flex-col md:flex-row print:flex-row justify-between items-start mb-12">
            <div className="flex items-center gap-6">
              {profile?.company_logo ? (
                <div className="border border-dashed border-[#d8b4fe] p-2 rounded-lg">
                  <img alt="Company Logo" className="h-16 w-16 object-contain" src={profile.company_logo} />
                </div>
              ) : (
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 whitespace-pre-line mt-1">
                  {profile?.company_address || '123 Business Rd.\nCity, Country'}
                </p>
              </div>
            </div>
            <div className="text-right mt-6 md:mt-0 print:mt-0">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-right text-sm">
                ...
                <span className="text-gray-700 font-medium">#{invoice.invoiceNumber || invoice.id?.substring(0, 8).toUpperCase()}</span>
                <span className="text-gray-500">Date:</span>
                <span className="text-gray-700 font-medium">{formatDate(issueDate)}</span>
                <span className="text-gray-500">Due date:</span>
                <span className="text-gray-700 font-medium">{formatDate(dueDate)}</span>
              </div>
            </div>
          </header>
          ...
          <section className="mb-8 border-t-[3px] border-[#a78bfa]">
            <div className="w-full">
              <div className="grid grid-cols-12 bg-[#f5f3ff] text-[#4c1d95] py-2 px-3">
                <div className="col-span-5 text-xs font-bold tracking-wide uppercase">ITEM DETAILS</div>
                <div className="col-span-2 text-xs font-bold tracking-wide uppercase text-right">PRICE</div>
                <div className="col-span-1 text-xs font-bold tracking-wide uppercase text-right">QTY</div>
                <div className="col-span-1 text-xs font-bold tracking-wide uppercase text-right">DISC</div>
                <div className="col-span-1 text-xs font-bold tracking-wide uppercase text-right">TAX</div>
                <div className="col-span-2 text-xs font-bold tracking-wide uppercase text-right">TOTAL</div>
              </div>
              ...
              <div className="divide-y divide-gray-100">
                {showGroups && invoice.groups && invoice.groups.length > 0 ? (
                  invoice.groups.map((group, gIdx) => (
                    <React.Fragment key={gIdx}>
                      ...
                      {group.items.map((item, iIdx) => (
                        <div key={iIdx} className="grid grid-cols-12 py-3 px-3 items-center">
                          <div className="col-span-5 pr-2">
                            <p className="font-medium text-gray-800 text-sm">{item.name}</p>
                            <p className="text-xs text-gray-500 mt-0.5">Item description or category</p>
                          </div>
                          <div className="col-span-2 text-sm text-gray-600 text-right">{formatMoney(item.unitPrice, sym)}</div>
                          <div className="col-span-1 text-sm text-gray-600 text-right">{item.quantity}</div>
                          <div className="col-span-1 text-sm text-gray-600 text-right">0%</div>
                          <div className="col-span-1 text-sm text-gray-600 text-right">0%</div>
                          <div className="col-span-2 text-sm font-medium text-gray-800 text-right">{formatMoney(item.quantity * item.unitPrice, sym)}</div>
                        </div>
                      ))}
                      {showGroupTotals && (
                        <div className="bg-[#fdfcff] flex justify-start px-3 py-2">
                          <span className="text-xs text-gray-500 font-medium mr-4">Group Subtotal</span>
                          <span className="text-xs font-semibold text-gray-700">{formatMoney(group.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0), sym)}</span>
                        </div>
                      )}
                    </React.Fragment>
                  ))
                  ...
                  items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 py-3 px-3 items-center">
                      <div className="col-span-5 pr-2">
                        <p className="font-medium text-gray-800 text-sm">{item.name}</p>
                        <p className="text-xs text-gray-500 mt-0.5">Item description or category</p>
                      </div>
            ...
            <div className="w-full sm:w-1/2 print:w-1/2 space-y-5">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    ...
                    {(invoice.bank_account_holder || profile?.bank_account_holder) && (
                      <p>Account: {invoice.bank_account_holder || profile?.bank_account_holder}</p>
                    )}
                    {(invoice.bank_account_number || profile?.bank_account_number) && (
                      <p>A/C: {invoice.bank_account_number || profile?.bank_account_number}</p>
                    )}
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 whitespace-pre-line">
                  {invoice.notes || 'Thank you for visiting us and making your first purchase! We\'re glad that you found what you were looking for. We look forward to seeing you again.'}
                </p>
              </div>

              <div>
                <h4 className="text-sm font-bold text-gray-800 mb-1">Terms and Conditions</h4>
                <p className="text-sm text-gray-500 whitespace-pre-line">
            ...
            <div className="w-full sm:w-[45%] print:w-[45%] flex flex-col">
              <div className="space-y-2 border-b border-gray-200 pb-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-800">{formatMoney(subtotal, sym)}</span>
                </div>
                {discountAmount > 0 && (
                ...
                <div className="flex justify-between text-sm font-medium pt-1 border-t border-gray-100">
                  ...
                  <span className="text-gray-800">{formatMoney(total, sym)}</span>
                </div>
                {amountPaid > 0 && (
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>Paid</span>
              ...
              <div className="flex justify-between items-center bg-gradient-to-r from-[#e9d5ff] to-[#f3e8ff] py-3 px-4 mt-3">
                ...
                <span className="font-bold text-[#4c1d95]">{formatMoney(balanceDue, sym)}</span>
              </div>
            </div>
          </section>
          ...
          <section className="mt-12 flex justify-end gap-16 print:mt-auto">
            {((profile?.signature_enabled ?? true) && (invoice.signature_url || profile?.signature_url || invoice.signatory_name || profile?.signatory_name)) && (
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm font-semibold text-gray-700">{profile?.company_name || 'Example Business'}</p>
                <p className="text-xs text-gray-500 mt-0.5">{formatDate(issueDate)}</p>
              </div>
            )}
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-xs text-gray-500 mt-0.5">{formatDate(dueDate)}</p>
            </div>
          </section>
