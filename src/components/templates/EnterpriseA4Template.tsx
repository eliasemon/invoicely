...
export function EnterpriseA4Template({ invoice, profile }: TemplateProps) {
  ...
  return (
    <div className="bg-[#f1f5f9] min-h-screen py-8 px-4 print:bg-white print:p-0 print:m-0 print:min-h-0" style={{ fontFamily: 'Geist, sans-serif' }}>
      <div className="max-w-[210mm] w-full min-h-[297mm] mx-auto bg-white shadow-sm border border-[#e2e8f0] print:shadow-none print:rounded-none print:border-none print:max-w-none print:w-full print:min-h-[297mm] print:my-0">
        {/* Top bar */}
        <div className="flex">
          <div className="w-2 bg-[#1e40af]"></div>
          <div className="flex-1 p-8 md:p-12">
            {/* Header */}
            <div className="flex justify-between items-start mb-12 pb-8 border-b border-[#e2e8f0]">
              <div>
            ...
            <table className="w-full mb-8 text-sm">
              <thead>
                <tr className="bg-[#1e40af] text-white">
                  <th className="text-left py-2.5 px-4 text-[10px] uppercase tracking-wider font-semibold">Description</th>
                  <th className="text-right py-2.5 px-4 text-[10px] uppercase tracking-wider font-semibold w-20">Qty</th>
                  <th className="text-right py-2.5 px-4 text-[10px] uppercase tracking-wider font-semibold w-28">Unit Price</th>
                  <th className="text-right py-2.5 px-4 text-[10px] uppercase tracking-wider font-semibold w-28">Amount</th>
                </tr>
              </thead>
              <tbody className="font-mono">
                {items.map((item, idx) => (
                  <tr key={idx} className={`border-b border-[#f1f5f9] ${idx % 2 === 1 ? 'bg-[#f8fafc]' : ''}`}>
                    ...
                    <td className="py-2.5 px-4 text-right text-[#64748b]">{item.quantity}</td>
                    <td className="py-2.5 px-4 text-right text-[#64748b]">{formatMoney(item.unitPrice, sym)}</td>
                    <td className="py-2.5 px-4 text-right font-semibold text-[#1e293b]">{formatMoney(item.quantity * item.unitPrice, sym)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {/* Footer */}
            <div className="mt-16 pt-6 border-t border-[#e2e8f0] text-xs text-[#94a3b8]">
