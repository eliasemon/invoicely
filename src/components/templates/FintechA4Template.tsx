...
export function FintechA4Template({ invoice, profile, showGroups, showGroupTotals, publicUrl }: TemplateProps) {
        ...
        <div className="p-6 p-6 print:p-6">
          ...
          <div className="flex flex-row print:flex-row justify-between items-start gap-3 mb-3">
            <div>
              ...
              <h1 className="text-sm font-bold text-[#0f172a]">{profile?.company_name || 'Your Company'}</h1>
              {profile?.email && <p className="text-[11px] text-[#475569] mt-1">{profile.email}</p>}
            ...
            <div className="flex flex-col items-end print:items-end gap-3 text-right print:text-right w-auto print:w-auto">
              ...
              <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl p-4 text-right print:text-right space-y-2.5 w-64 print:w-64 shadow-sm">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-[#0f172a] text-xs">{formatDate(issueDate)}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
            ...
            <div className="overflow-x-auto mb-2">
              ...
              <thead>
                <tr className="border-b border-[#e2e8f0] bg-[#f1f5f9]">
                  ...
                  <th className="text-right py-1 px-2 text-[10px] text-[#64748b] uppercase tracking-wider w-16">Qty</th>
                  <th className="text-right py-1 px-2 text-[10px] text-[#64748b] uppercase tracking-wider w-24">Rate</th>
                  <th className="text-right py-1 px-2 text-[10px] text-[#64748b] uppercase tracking-wider w-28">Amount</th>
                </tr>
              </thead>
                    ...
                    <React.Fragment key={gIdx}>
                      ...
                      {group.items.map((item, iIdx) => (
                        <tr key={iIdx} className="border-b border-[#e2e8f0] bg-white hover:bg-[#f1f5f9] transition-colors">
                          <td className="py-1 px-2 text-[#0f172a]" style={{ fontFamily: 'Geist, sans-serif' }}>{item.name}</td>
                          <td className="py-1 px-2 text-right text-[#475569]">{item.quantity}</td>
                          <td className="py-1 px-2 text-right text-[#475569]">{formatMoney(item.unitPrice, sym)}</td>
                          <td className="py-1 px-2 text-right text-[#16a34a] font-semibold">{formatMoney(item.quantity * item.unitPrice, sym)}</td>
                        </tr>
                      ))}
          ...
          <div className="flex justify-between items-end w-full">
            ...
            <div className="w-64 print:w-64 bg-[#f8fafc] border border-[#e2e8f0] rounded-xl p-4">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between py-1 text-xs text-[#64748b] font-mono border-b border-[#e2e8f0]/60"><span>Discount {invoice.discount_type === 'percentage' ? `(${invoice.discount_value}%)` : ''}</span><span>-{formatMoney(discountAmount, sym)}</span></div>
              )}
              ...
              {shippingCost > 0 && (
                <div className="flex justify-between py-1 text-xs text-[#64748b] font-mono border-b border-[#e2e8f0]/60"><span>Shipping</span><span>+{formatMoney(shippingCost, sym)}</span></div>
              )}
