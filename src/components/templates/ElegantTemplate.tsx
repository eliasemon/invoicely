...
export function ElegantTemplate({ invoice, profile, isPreview, showGroups, showGroupTotals, publicUrl }: TemplateProps) {
    ...
    <div className="bg-[#cbdbf5] min-h-screen text-[#0b1c30] py-8 px-4 print:bg-white print:p-0 print:m-0 print:min-h-0 print:w-[210mm]" style={{ fontFamily: 'Hanken Grotesk, sans-serif' }}>
      ...
      <main className="w-full max-w-[210mm] min-h-[297mm] mx-auto bg-white shadow-[0_4px_40px_rgba(11,28,48,0.06)] rounded-sm overflow-hidden p-6 md:p-6 print:p-6 relative print:shadow-none print:rounded-none print:border-none print:w-[210mm] print:max-w-[210mm] print:mx-0 print:min-h-[297mm] print:my-0">
        ...
        <section className="mb-3 border-l-2 border-[#565e74] pl-4">
          ...
          <p className="text-[11px] font-semibold mb-1" style={{ fontFamily: 'Work Sans, sans-serif' }}>{invoice.clientName}</p>
        ...
        <section className="mb-3">
          <div className="w-full">
            ...
            {showGroups && invoice.groups && invoice.groups.length > 0 ? (
              invoice.groups.map((group, gIdx) => (
                <React.Fragment key={gIdx}>
                  ...
                  {group.items.map((item, iIdx) => (
                    <div key={iIdx} className="grid grid-cols-12 gap-2 py-2 border-b border-[#dce9ff] items-start col-span-12">
              ...
              items.map((item, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 py-2 border-b border-[#dce9ff] items-start">
                  <div className="col-span-6">
                    <p className="font-medium">{item.name}</p>
                  </div>
                  <div className="col-span-2 text-right text-sm text-[#565e74]" style={{ fontFamily: 'Geist, monospace' }}>{item.quantity.toFixed(2)}</div>
