...
export function ElegantTemplate({ invoice, profile, isPreview }: TemplateProps) {
  ...
  return (
    <div className="bg-[#cbdbf5] min-h-screen text-[#0b1c30] py-8 px-4 print:bg-white print:p-0 print:m-0 print:min-h-0" style={{ fontFamily: 'Hanken Grotesk, sans-serif' }}>
      ...
      <main className="w-full max-w-[210mm] min-h-[297mm] mx-auto bg-white shadow-[0_4px_40px_rgba(11,28,48,0.06)] rounded-sm overflow-hidden p-8 md:p-16 relative print:shadow-none print:rounded-none print:border-none print:max-w-none print:w-full print:min-h-[297mm] print:my-0">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16">
          <div>
            {profile?.company_logo && (
              <img alt="Company Logo" className="max-h-28 max-w-[240px] mb-6 object-contain w-auto h-auto" src={profile.company_logo} />
            )}
          ...
          <div className="text-left md:text-right">
            <h2 className="text-2xl font-semibold text-[#76777d] tracking-wider mb-4" style={{ fontFamily: 'Work Sans, sans-serif' }}>INVOICE</h2>
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-right">
              <span className="text-[12px] text-[#76777d] text-left md:text-right">Invoice No.</span>
              <span className="text-sm" style={{ fontFamily: 'Geist, monospace' }}>{invoice.invoiceNumber || invoice.id?.substring(0, 8).toUpperCase()}</span>
              <span className="text-[12px] text-[#76777d] text-left md:text-right">Issue Date</span>
              <span className="text-sm" style={{ fontFamily: 'Geist, monospace' }}>{formatDate(invoice.createdAt)}</span>
              <span className="text-[12px] text-[#76777d] text-left md:text-right">Due Date</span>
        ...
        {/* Bill To */}
        <section className="mb-16 border-l-2 border-[#565e74] pl-6">
        ...
        <section className="mb-12">
          <div className="w-full">
            <div className="grid grid-cols-12 gap-4 border-b border-[#c6c6cd] pb-4 mb-4">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-4 py-4 border-b border-[#dce9ff] items-start">
        ...
        {/* Totals */}
        <section className="flex flex-col items-end mb-24">
        ...
        {/* Footer */}
        <footer className="flex flex-col md:flex-row justify-between items-end gap-12 mt-auto w-full pt-12 border-t border-[#dce9ff]">
