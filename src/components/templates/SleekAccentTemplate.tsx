...
export function SleekAccentTemplate({ invoice, profile, isPreview, showGroups, showGroupTotals, publicUrl }: TemplateProps) {
  ...
  return (
    <div className="bg-[#f2f4f8] min-h-screen text-[#1a1c21] py-8 px-4 print:bg-white print:p-0 print:m-0 print:min-h-0 print:w-[210mm]" style={{ fontFamily: 'Hanken Grotesk, sans-serif' }}>
      {/* Floating Actions */}
      {!isPreview && (
        <div className="fixed top-6 right-6 md:right-12 z-50 flex gap-2 print:hidden">
          <button onClick={() => window.print()} className="bg-[#0b1b3d] text-white px-6 py-2.5 rounded-full shadow-md text-xs flex items-center gap-2 hover:bg-[#1a2b52] transition-colors">
      ...
      <main className="w-full max-w-[210mm] min-h-[297mm] mx-auto bg-white shadow-[0_4px_40px_rgba(0,0,0,0.08)] overflow-hidden flex flex-col relative print:shadow-none print:w-[210mm] print:max-w-[210mm] print:mx-0 print:min-h-[297mm] print:my-0">
        ...
        <div className="flex-1 flex flex-col p-8 md:p-12 print:p-10 relative z-10">
          {/* Header */}
          <header className="flex flex-col md:flex-row print:flex-row justify-between items-start mb-10">
            <div className="flex items-center gap-4">
              {profile?.company_logo ? (
                <img alt="Company Logo" className="h-16 w-auto object-contain" src={profile.company_logo} />
              ) : null}
              <h1 className="text-2xl font-bold text-[#0b1b3d]" style={{ fontFamily: 'Work Sans, sans-serif' }}>
                {profile?.company_name || 'Your Company'}
              </h1>
            </div>
            <div className="text-right mt-4 md:mt-0 print:mt-0">
              <h2 className="text-2xl font-bold text-[#0b1b3d] tracking-wide mb-4" style={{ fontFamily: 'Work Sans, sans-serif' }}>INVOICE #</h2>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-right text-sm">
                <span className="text-gray-500 font-medium">INVOICE # :</span>
                <span className="font-semibold">{invoice.invoiceNumber || invoice.id?.substring(0, 8).toUpperCase()}</span>
          ...
          {/* Addresses */}
          <section className="flex flex-col md:flex-row print:flex-row justify-between mb-8">
            <div className="mb-4 md:mb-0 print:mb-0">
              <h3 className="text-sm font-bold text-[#0b1b3d] mb-2 uppercase tracking-wide">FROM</h3>
              <p className="text-sm font-bold">{profile?.company_name || 'Your Company'}</p>
            ...
            <div className="text-left md:text-right print:text-right">
              <h3 className="text-sm font-bold text-[#0b1b3d] mb-2 uppercase tracking-wide">BILL TO</h3>
          ...
          <section className="mb-8">
            <div className="w-full border border-gray-200">
              <div className="grid grid-cols-12 bg-[#0b1b3d] text-white py-3 px-4 rounded-t-sm">
                <div className="col-span-4 text-xs font-semibold tracking-wider text-center">DESCRIPTION</div>
                <div className="col-span-2 text-xs font-semibold tracking-wider text-center">QTY</div>
                <div className="col-span-2 text-xs font-semibold tracking-wider text-center">PRICE</div>
                <div className="col-span-1 text-xs font-semibold tracking-wider text-center">TAX</div>
                <div className="col-span-1 text-xs font-semibold tracking-wider text-center text-nowrap text-ellipsis overflow-hidden">DISC.</div>
                <div className="col-span-2 text-xs font-semibold tracking-wider text-center">AMOUNT</div>
              </div>
              ...
              <div className="divide-y divide-gray-200">
                {showGroups && invoice.groups && invoice.groups.length > 0 ? (
                  invoice.groups.map((group, gIdx) => (
                    <React.Fragment key={gIdx}>
                      ...
                      {group.items.map((item, iIdx) => (
                        <div key={iIdx} className={`grid grid-cols-12 py-3 px-4 ${iIdx % 2 !== 0 ? 'bg-orange-50/30' : 'bg-white'} items-center`}>
                          <div className="col-span-4 text-sm font-medium text-center">{item.name}</div>
                          <div className="col-span-2 text-sm text-center">{item.quantity}</div>
                          <div className="col-span-2 text-sm text-center">{formatMoney(item.unitPrice, sym)}</div>
                          <div className="col-span-1 text-sm text-center">{formatMoney(tax, sym)}</div>
                          <div className="col-span-1 text-sm text-center">{sym}0</div>
                          <div className="col-span-2 text-sm font-medium text-center">{formatMoney(item.quantity * item.unitPrice, sym)}</div>
                        </div>
                      ))}
                      {showGroupTotals && (
                        <div className="bg-gray-50 flex justify-start px-4 py-2 border-t border-gray-200">
                          <span className="text-xs text-gray-500 font-bold mr-4 uppercase">Group Subtotal</span>
                          <span className="text-xs font-bold text-[#0b1b3d]">{formatMoney(group.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0), sym)}</span>
                        </div>
                      )}
                    </React.Fragment>
                  ))
                  ...
                  items.map((item, idx) => (
                    <div key={idx} className={`grid grid-cols-12 py-3 px-4 ${idx % 2 !== 0 ? 'bg-orange-50/30' : 'bg-white'} items-center`}>
                      <div className="col-span-4 text-sm font-medium text-center">{item.name}</div>
                      <div className="col-span-2 text-sm text-center">{item.quantity}</div>
                      <div className="col-span-2 text-sm text-center">{formatMoney(item.unitPrice, sym)}</div>
                      <div className="col-span-1 text-sm text-center">{formatMoney(tax, sym)}</div>
          ...
          {/* Payment & Totals */}
          <section className="flex flex-col sm:flex-row print:flex-row justify-between items-start mt-auto gap-8">
            <div className="w-full sm:w-1/2 print:w-1/2">
              <h4 className="text-xs font-bold text-[#0b1b3d] uppercase mb-2">PAYMENT METHOD</h4>
              {((profile?.bank_enabled ?? true) && (invoice.bank_name || profile?.bank_name)) ? (
                <div className="text-xs text-gray-500 space-y-1">
                  ...
                  {(invoice.bank_account_holder || profile?.bank_account_holder) && (
                    <p>Account Name: {invoice.bank_account_holder || profile?.bank_account_holder}</p>
                  )}
                  {(invoice.bank_account_number || profile?.bank_account_number) && (
                    <p>Account No: {invoice.bank_account_number || profile?.bank_account_number}</p>
                  )}
                  {(invoice.bank_swift || profile?.bank_swift) && (
                    <p>SWIFT/IFSC: {invoice.bank_swift || profile?.bank_swift}</p>
                  )}
                </div>
              ) : (
                <p className="text-xs text-gray-500">Please refer to the payment instructions provided separately.</p>
              )}

              <div className="mt-8">
                <h4 className="text-xs font-bold text-[#0b1b3d] uppercase mb-2">TERMS & CONDITIONS</h4>
                <p className="text-xs text-gray-400 whitespace-pre-line leading-relaxed">
                  {invoice.notes || 'Payment is due within the specified dates. Late payments may incur additional charges.'}
                </p>
              </div>
            ...
            <div className="w-full sm:w-1/2 print:w-1/2 flex flex-col items-end">
              <div className="w-[80%]">
                ...
                <div className="flex justify-between items-center bg-[#0b1b3d] text-white py-2 px-3 mt-2 rounded-sm shadow-sm">
                  ...
                  <span className="text-lg font-bold">{formatMoney(total, sym)}</span>
                </div>
              ...
              {((profile?.signature_enabled ?? true) && (invoice.signature_url || profile?.signature_url || invoice.signatory_name || profile?.signatory_name)) && (
                <div className="mt-12 flex flex-col items-center">
                  ...
                  <p className="text-xs font-bold text-[#0b1b3d] uppercase tracking-wide">SIGNATURE</p>
                  <p className="text-[10px] text-gray-500 mt-1">{invoice.signatory_name || profile?.signatory_name || 'Authorized Signatory'}</p>
                </div>
              )}
            </div>
          </section>
